...
- `payroll_periods` owns the period state. `payroll_items` stores employee-name, currency and base-salary snapshots. `payroll_adjustments` stores typed integer-minor-unit changes with a reason and creator.

Database checks enforce valid enums, ordered intervals, progress bounds and unique period/assignment/link invariants. Query indexes cover production status/date/priority, attendee lookup, task assignee/due/status/production, meeting time, calendar interval/link, and payroll period/item access.
