...
class PostgresApplicationTest {
  ...
  @Autowired
  @Qualifier("phase2DemoData")
  CommandLineRunner phase2Seed;
  ...
  @Test
  void realApplicationValidatesSchemaAndBootstrapIsIdempotent() throws Exception {
    assertSeed();
    seed.run();
    phase2Seed.run();
    assertSeed();
    assertThat(
            jdbc.queryForObject(
                "select count(*) from employees where salary_currency='INR'", Integer.class))
        .isEqualTo(12);
    assertPhase2Seed();
    verifyHttpWorkflows();
  }
  ...
  private void verifyHttpWorkflows() throws Exception {
    ...
    token =
        call(
                ...
                "POST",
                "/auth/login",
                "{\"email\":\"owner@saproduction.local\",\"password\":\"SADemo!2026\"}")
            .path("token")
            .asText();
    assertThat(call(client, "GET", "/auth/me", null).path("email").asText())
        .isEqualTo("owner@saproduction.local");
    ...
    for (var row : day.path("rows"))
      assertThat(row.path("record").path("employeeId").asText())
          .isEqualTo(row.path("employee").path("id").asText());
    ...
    assertThat(call(client, "GET", "/employees/" + id, null).path("roleTitle").asText())
        .isEqualTo("Senior Editor");
    call(client, "PUT", "/attendance/" + id + "/" + today, "{\"status\":\"ABSENT\"}");
    assertThat(
            call(client, "GET", "/employees/" + id + "/attendance", null)
                .path("records")
                .get(0)
                .path("status")
                .asText())
    ...
    String leave =
        "{\"employeeId\":\""
            ...
            + "\",\"endDate\":\""
            + today
            + "\",\"leaveType\":\"Personal\",\"reason\":\"Release verification\"}";
    String approved = call(client, "POST", "/leave-requests", leave).path("id").asText();
    assertThat(
            call(client, "POST", "/leave-requests/" + approved + "/approve", "{}")
                .path("status")
                .asText())
        .isEqualTo("APPROVED");
    assertThat(
            call(client, "GET", "/employees/" + id + "/attendance", null)
                ...
                .asText())
        .isEqualTo("LEAVE");
    assertThat(call(client, "GET", "/employees/" + id, null).path("status").asText())
        .isEqualTo("ON_LEAVE");
    String rejected = call(client, "POST", "/leave-requests", leave).path("id").asText();
    assertThat(
            call(client, "POST", "/leave-requests/" + rejected + "/reject", "{}")
                .path("status")
                .asText())
        .isEqualTo("REJECTED");
    assertThat(jdbc.queryForObject("select count(*) from audit_logs", Integer.class))
  ...
  private void assertSeed() {
    ...
    assertThat(jdbc.queryForObject("select count(*) from attendance_records", Integer.class))
        .isEqualTo(12);
    assertThat(jdbc.queryForObject("select count(*) from leave_requests", Integer.class))
        .isEqualTo(1);
  }
  ...
  private void assertPhase2Seed() {
    ...
    assertThat(jdbc.queryForObject("select count(*) from calendar_events", Integer.class))
        .isEqualTo(34);
    assertThat(jdbc.queryForObject("select count(*) from payroll_periods", Integer.class))
        .isEqualTo(2);
    assertThat(
            jdbc.queryForObject(
                "select count(*) from payroll_periods where status='LOCKED'", Integer.class))
        .isEqualTo(1);
  }
}
