...
class EmployeeServiceTest {
  @Test
  void createPersistsMinorUnitsAndWritesAudit() {
    EmployeeRepository repository = mock(EmployeeRepository.class);
    AuditService audit = mock(AuditService.class);
    when(repository.existsByEmployeeCodeIgnoreCase("SA-100")).thenReturn(false);
    when(repository.saveAndFlush(any(Employee.class)))
        .thenAnswer(
            invocation -> {
    ...
    EmployeeService service =
        new EmployeeService(repository, audit, mock(DomainEventService.class));
            ...
            argThat(
                value ->
                    value instanceof java.util.Map<?, ?> map
                        ...
                        && !map.containsKey("baseSalaryMinor")));
  }

  @Test
  void updateAuditsSalaryChanges() {
    UUID id = UUID.randomUUID();
    Employee existing = employee(id);
    existing.baseSalaryMinor = 400_000L;
    EmployeeRepository repository = mock(EmployeeRepository.class);
    AuditService audit = mock(AuditService.class);
    ...
    verify(audit)
        .record(
            ...
            id.toString(),
            java.util.Map.of("amountMinor", 400_000L, "currency", "INR"),
