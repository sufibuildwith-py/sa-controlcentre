...
class EmployeeValidationTest {
  ...
  @Test
  void rejectsNegativeMinorUnitSalaryAndInvalidPhone() {
    var input =
        new EmployeeDtos.Input(
            ...
            null);
    var fields =
        validator.validate(input).stream().map(v -> v.getPropertyPath().toString()).toList();
    assertThat(fields).contains("phone", "baseSalaryMinor");
  }

  @Test
  void acceptsIntegerMinorUnitSalary() {
    var input =
        new EmployeeDtos.Input(
            "SA-901",
            "Test",
