...
class PostgresMigrationTest {
  ...
  @Test void payrollPaymentRequiresPositiveAmountAndKnownMethod() throws Exception {
    String item=java.util.UUID.randomUUID().toString();
    try(Connection c=DriverManager.getConnection(postgres.getJdbcUrl(),postgres.getUsername(),postgres.getPassword())){UUIDs ids=insertOwnerAndEmployee(c);try(PreparedStatement p=c.prepareStatement("insert into payroll_periods(year,month,status) values (2028,1,'APPROVED') returning id")){ResultSet r=p.executeQuery();r.next();String period=r.getString(1);try(PreparedStatement i=c.prepareStatement("insert into payroll_items(id,payroll_period_id,employee_id,employee_name_snapshot,salary_currency,base_salary_minor,net_salary_minor,payment_status) values (?::uuid,?::uuid,?::uuid,'Test','INR',4200000,4200000,'UNPAID')")){i.setString(1,item);i.setString(2,period);i.setString(3,ids.employee());i.executeUpdate();}}}
  ...
  private UUIDs insertOwnerAndEmployee(Connection c)throws Exception{String user=java.util.UUID.randomUUID().toString(),employee=java.util.UUID.randomUUID().toString(),suffix=user.substring(0,8);try(PreparedStatement p=c.prepareStatement("insert into users(id,email,password_hash,display_name,role) values (?::uuid,?,'hash','Owner','OWNER')")){p.setString(1,user);p.setString(2,"test-"+suffix+"@sa.local");p.executeUpdate();}try(PreparedStatement p=c.prepareStatement("insert into employees(id,employee_code,first_name,display_name,phone,role_title,department,employment_type,joining_date,base_salary_minor,salary_currency,status) values (?::uuid,?,'Test','Test',?,'Editor','Post','FULL_TIME',current_date,4200000,'INR','ACTIVE')")){p.setString(1,employee);p.setString(2,"SA-"+suffix);p.setString(3,"+91"+Math.abs(user.hashCode()));p.executeUpdate();}return new UUIDs(user,employee);}
  ...
  private void insertPayment(String item,long amount,String method)throws Exception{try(Connection c=DriverManager.getConnection(postgres.getJdbcUrl(),postgres.getUsername(),postgres.getPassword());PreparedStatement p=c.prepareStatement("insert into payroll_payments(payroll_item_id,amount_minor,paid_at,payment_method) values (?::uuid,?,now(),?)")){p.setString(1,item);p.setLong(2,amount);p.setString(3,method);p.executeUpdate();}}
