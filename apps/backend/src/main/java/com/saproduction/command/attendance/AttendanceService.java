...
public class AttendanceService {
  ...
  private final AttendanceRepository attendance; private final EmployeeRepository employees; private final EmployeeService employeeService; private final UserRepository users; private final AuditService audit;private final DomainEventService events;
  public AttendanceService(AttendanceRepository attendance,EmployeeRepository employees,EmployeeService employeeService,UserRepository users,AuditService audit,DomainEventService events){this.attendance=attendance;this.employees=employees;this.employeeService=employeeService;this.users=users;this.audit=audit;this.events=events;}
  ...
  @Transactional public RecordView put(UUID employeeId,LocalDate date,Input input){
    ...
    RecordView after=view(record);audit.record("ATTENDANCE",before==null?"ATTENDANCE_CREATED":"ATTENDANCE_CHANGED",record.id.toString(),before,after);events.emit("ATTENDANCE_RECORDED","ATTENDANCE",record.id,Map.of("employeeId",employeeId,"date",date.toString(),"status",record.status.name()));return after;
  }
