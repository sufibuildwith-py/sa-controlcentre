...
public class MeetingService {
  ...
  private final MeetingRepository meetings;private final MeetingNoteRepository notes;private final CalendarService calendar;private final WorkTaskService tasks;private final JdbcTemplate jdbc;private final AuditService audit;private final DomainEventService events;
  public MeetingService(MeetingRepository meetings,MeetingNoteRepository notes,CalendarService calendar,WorkTaskService tasks,JdbcTemplate jdbc,AuditService audit,DomainEventService events){this.meetings=meetings;this.notes=notes;this.calendar=calendar;this.tasks=tasks;this.jdbc=jdbc;this.audit=audit;this.events=events;}
  ...
  private void emitMeeting(String type,Meeting m,List<UUID> employeeIds){if(employeeIds.isEmpty())return;events.emit(type,"MEETING",m.id,Map.of("employeeIds",employeeIds,"category","MEETINGS","relatedType","MEETING","relatedId",m.id,"requiresResponse",true,"bodyPreview",m.title+" · "+m.startsAt,"variables",Map.of("meeting",m.title,"startsAt",m.startsAt.toString(),"location",m.location==null?"To be confirmed":m.location)));}
  private void emitReminder(Meeting m,List<UUID> employeeIds){if(employeeIds.isEmpty())return;Instant sendAt=m.startsAt.minusSeconds(7200);events.emitAt("MEETING_REMINDER","MEETING",m.id,Map.of("employeeIds",employeeIds,"category","MEETINGS","relatedType","MEETING","relatedId",m.id,"bodyPreview","Reminder · "+m.title,"sendAt",sendAt.toString(),"variables",Map.of("meeting",m.title,"startsAt",m.startsAt.toString())),sendAt.isAfter(Instant.now())?sendAt:Instant.now());}
