...
public class SchedulingConflictService {
  ...
  public SchedulingConflictService(JdbcTemplate jdbc){this.jdbc=jdbc;}
  public List<Conflict> find(UUID employeeId,Instant start,Instant end,UUID excludedEvent){
    String sql=excludedEvent==null?"""
      ...
      """;
    Object[] args=excludedEvent==null?new Object[]{employeeId,java.sql.Timestamp.from(end),java.sql.Timestamp.from(start)}:new Object[]{employeeId,java.sql.Timestamp.from(end),java.sql.Timestamp.from(start),excludedEvent};
    return jdbc.query(sql,(ResultSet r,int n)->new Conflict(r.getObject(1,UUID.class),r.getString(2),r.getTimestamp(3).toInstant(),r.getTimestamp(4).toInstant()),args);
  }
}
