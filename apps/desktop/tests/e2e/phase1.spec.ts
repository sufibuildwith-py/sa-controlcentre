...
async function mockApi(page:Page,mock:MockState){await page.route('**/api/v1/**',async route=>{
  ...
  if(path.endsWith('/dashboard')&&method==='GET')return fulfill(route,{team:{employees:1,present:1,late:0,absent:0,leave:0,incomplete:0},today:[{id:'event-1',type:'INTERNAL',title:'Daily operations review',startsAt:fixedNow.toISOString(),endsAt:new Date(fixedNow.getTime()+3600000).toISOString()}],productions:[{id:'production-1',title:'Sharma Wedding',status:'PRODUCTION',progressPercent:78,eventDate:'2026-09-26'}],workload:[{employeeId:baseEmployee.id,employeeName:baseEmployee.displayName,active:2,overdue:0}],payroll:{id:'payroll-1',year:2026,month:9,status:'CALCULATED',totalMinor:4200000,paidMinor:0,pendingMinor:4200000},attention:[{code:'ATTENDANCE_COMPLETE',title:'Attendance is complete',detail:'All active employees have a status.',tone:'neutral',href:'/attendance'}],communications:{queued:0,sent:0,delivered:0,read:0,failed:0,awaitingResponse:0}});
  if(path.endsWith('/notification-rules')&&method==='GET')return fulfill(route,[]);
  if(path.endsWith('/demo/messages')&&method==='GET')return fulfill(route,{summary:{delivered:0,read:0,awaitingResponse:0,failed:0,queued:0,sentToday:0},messages:[]});
