...
const dock = [
  ...
  { label: "Work", to: "/work", icon: CheckSquare },
...
export function AppShell({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    document
      ...
      ?.setAttribute("content", theme === "pearl" ? "#ecece8" : "#1e1e1e");
  }, [theme]);
  useEffect(() => {
  ...
  useEffect(() => {
    workspace.current?.scrollTo({ top: 0, left: 0 });
  }, [location.pathname]);
  return (
    <div className="app-frame">
      <div className="app-shell">
        ...
        <QuickCreate />
        <SACommandPalette onNavigate={navigate} />
        <EmployeeForm />
      </div>
    </div>
  );
...
export function OperationalDock() {
  const setPaletteOpen = useUiStore((s) => s.setPaletteOpen);
  return (
    <nav className="operational-dock" aria-label="Operational shortcuts">
      ...
      {dock.map((item) => {
        ...
        if (item.action === "search")
          return (
            <Tooltip key={item.label} content={`${item.label} · Ctrl/Cmd K`}>
              <button
...
export const WorkspaceShell = forwardRef<HTMLElement, PropsWithChildren>(
  ({ children }, ref) => {
    const location = useLocation();
    return (
      <main ref={ref} className="workspace-shell">
        <div key={location.pathname} className="workspace-page">
          {children}
        </div>
      </main>
    );
...
export function WorkspaceHeader({
  title,
  subtitle,
  owner = import.meta.env.VITE_DESKTOP_RELEASE === "true"
    ? "Azeem Khan"
    : "Owner",
}: {
  title: string;
  ...
  const leave = async (revoke: boolean) => {
    try {
      if (revoke) await api("/auth/logout", { method: "POST" });
    } finally {
  ...
  const logout = useMutation({ mutationFn: () => leave(true) });
  return (
    <header className="workspace-header">
      ...
      <div className="header-actions">
        ...
        <div className="header-menu">
          ...
          {attentionOpen && (
            <div className="header-popover attention-popover">
        ...
        <div className="header-menu">
          ...
          {ownerOpen && (
            <div className="header-popover owner-menu">
