...
export function FinancePage() {
  ...
  return <div className="finance-page">
    ...
    {tab === "OVERVIEW" && (overview.isPending ? <SkeletonCard/> : overview.isError || !overview.data ? <EmptyState title="Finance unavailable" description="The financial position could not be loaded." action={<SAButton onClick={() => overview.refetch()}>Try again</SAButton>}/> : <>
      ...
      {import.meta.env.VITE_APP_MODE === "demo" && workbookPurchases.data?.available && <SABentoCard><span className="eyebrow">Purchases · workbook-backed</span><div className="finance-kpis"><div><span>LED / Sound outstanding</span><strong>{rupees((workbookPurchases.data?.books ?? []).reduce((sum: number, book: { outstanding: number }) => sum + Number(book.outstanding), 0))}</strong></div><div><span>Equipment references</span><strong>{workbookPurchases.data?.accessories.records}</strong></div></div><Link to="/finance?workbookPurchase=1">View purchases & equipment →</Link><small>Purchases and physical custody remain separate.</small></SABentoCard>}
    ...
    {tab === "PARTIES" && <section className="finance-section"><div className="finance-section-head"><div><span className="eyebrow">Posted Finance · separate from workbook history</span><h2>Parties & receivables</h2></div><div className="finance-actions"><SAButton onClick={() => open("party")}>Add party</SAButton><SAButton onClick={() => open("charge")}>Charge</SAButton><SAButton onClick={() => open("party-receipt")}>Receipt</SAButton></div></div><input className="finance-search" placeholder="Search parties" aria-label="Search parties" value={search} onChange={e => {setSearch(e.target.value);setPage(0);}}/>{parties.isPending ? <SkeletonCard/> : parties.isError ? <EmptyState title="Parties unavailable" description="Could not load counterparty ledgers."/> : <><Table headers={["Party", "Role", "Charged", "Received", "Outstanding", "Action"]}>{parties.data?.items.map(p => <tr key={p.id}><td>{p.displayName}</td><td>{p.role}</td><td>{rupees(p.charged)}</td><td>{rupees(p.received)}</td><td>{signed(p.charged - p.received)}</td><td><SAButton size="sm" onClick={() => navigate(`/billing?counterpartyId=${p.id}`)}>Create bill</SAButton></td></tr>)}</Table><PageNav page={page} total={parties.data?.total ?? 0} onPage={setPage}/></>}</section>}
