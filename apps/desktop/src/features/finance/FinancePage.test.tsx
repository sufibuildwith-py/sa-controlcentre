...
const mock = vi.hoisted(() => ({
  overview: vi.fn(), config: vi.fn(), reconciliation: vi.fn(), productions: vi.fn(), production: vi.fn(),
  employees: vi.fn(), employee: vi.fn(), counterparties: vi.fn(), invoices: vi.fn(), purchases: vi.fn(),
  transactions: vi.fn(), transaction: vi.fn(), post: vi.fn(), reverse: vi.fn(), rebuild: vi.fn(),
  previewWorkbook: vi.fn(), validateWorkbook: vi.fn(),
  workbookOwners: vi.fn(), workbookEmployees: vi.fn(), workbookPartySummary: vi.fn(),
}));
...
function setup() {
  mock.overview.mockResolvedValue({ overallResult: -100, received: 200, incurredExpense: 300, contracted: 400,
    receivables: 200, postedCount: 2, accounts: [
      { id: "a", code: "AZ-2", displayName: "Azeem", position: -50 },
      { id: "b", code: "AK-2", displayName: "Akash", position: 25 },
    ] });
  mock.config.mockResolvedValue({ accounts: [
    { id: "a", code: "AZ-2", displayName: "Azeem", position: -50 },
    { id: "b", code: "AK-2", displayName: "Akash", position: 25 },
  ], expenseCategories: [{ id: "c", code: "FOOD", displayName: "Food" }], profitSplit: [] });
  mock.productions.mockResolvedValue({ items: [], page: 0, size: 50, total: 0 });
  mock.employees.mockResolvedValue({ items: [], page: 0, size: 50, total: 0 });
  mock.transactions.mockResolvedValue({ items: [], page: 0, size: 50, total: 0 });
  ...
  render(<QueryClientProvider client={queryClient}><MemoryRouter><FinancePage/></MemoryRouter></QueryClientProvider>);
}
...
describe("Finance command surface", () => {
  ...
  it("does not permit an expense without amount, evidence and deliberate payer selection", async () => {
    ...
    expect(screen.getByRole("button", { name: "Post transaction" })).toBeDisabled();
    await user.type(screen.getByLabelText("Amount · INR"), "6500");
    await user.type(screen.getByLabelText("Description / evidence"), "Food for crew");
    expect(screen.getByRole("button", { name: "Post transaction" })).toBeDisabled();
    await user.selectOptions(await screen.findByLabelText("Paid by"), "AZ-2");
    expect(screen.getByRole("button", { name: "Post transaction" })).toBeEnabled();
  });

  it("shows a useful empty transaction state", async () => {
    setup();
    await userEvent.setup().click(screen.getByRole("button", { name: "Transactions" }));
