...
describe("employeeFormSchema", () => {
  it("accepts production-safe employee values", () =>
    expect(employeeFormSchema.safeParse(valid).success).toBe(true));
  it("rejects a negative salary and malformed phone", () => {
    ...
    if (!result.success)
      expect(result.error.issues.map((i) => i.path[0])).toEqual(
        expect.arrayContaining(["baseSalaryRupees", "phone"]),
      );
  });
});
